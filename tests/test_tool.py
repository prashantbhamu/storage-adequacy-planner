...
def synthetic_run(hours: int = 72):
    ...
    solar = 6.0 * np.exp(-((hour - 13.0) / 3.0) ** 2)
    supply = demand + solar - 3.0 * np.exp(-((hour - 21.0) / 2.0) ** 2)
    spec = StorageSpec(2.0, 2.0, 8.0, 0.9, 1.0)
    return timestamps, demand, supply, solar, spec
...
def test_optimizer_constraints_and_objective_improvement() -> None:
    timestamps, demand, supply, solar, spec = synthetic_run()
    result = optimize_storage(timestamps, demand, supply, solar, spec)
...
def test_clean_xlsx_export_contains_two_sheets() -> None:
    ...
    assert workbook["Hourly Results"]["H2"].value == result.dispatch[0]
    assert workbook["Summary"]["B10"].value == "2.0.0"
...
def test_xlsx_api_workflow_and_download() -> None:
    ...
    solar = 5.0 * np.exp(-((hour - 13.0) / 3.0) ** 2)
    supply = demand + solar - 2.5 * np.exp(-((hour - 21.0) / 2.0) ** 2)
    source = pd.DataFrame(
        {
    ...
    preview = client.post(
        "/api/preview",
        files={"file": ("february.xlsx", payload, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")},
    )
    ...
    assert "solar" not in " ".join(result["method"]["objective"]).lower()
