...
export function DurationCurve({ hourly }: { hourly: HourlyResult[] }) {
  ...
  const low = Math.min(points[points.length - 1].before, points[points.length - 1].after);
    ...
    <section className="panel" aria-labelledby="duration-title">
      <header className="panel-head">
        <div>
          ...
          <p>Every hour sorted from largest surplus to deepest shortage.</p>
        </div>
      ...
      <ResponsiveContainer width="100%" height={230}>
        <ComposedChart data={points} margin={{ top: 8, right: 16, bottom: 0, left: 0 }}>
          ...
          {low < 0 ? <ReferenceArea y1={low * 1.08} y2={0} fill="var(--c-deficit)" fillOpacity={0.07} ifOverflow="extendDomain" /> : null}
            ...
            content={({ active, payload }) => {
              ...
              return (
                <div className="chart-tooltip">
                  ...
                  <div><span><i className="line-key is-before" />Before</span><b>{num(point.before, 2)} GW</b></div>
                  <div><span><i className="line-key is-after" />After</span><b>{num(point.after, 2)} GW</b></div>
                </div>
              );
            }}
          />
